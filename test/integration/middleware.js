'use strict'

import { deepEqual, equal, ok } from 'assert'
import mongoose from 'mongoose'
import request, { del, get, post } from 'request'
import { assert as _assert, spy } from 'sinon'
import * as erm from '../../src/express-restify-mongoose'
import dbSetup from './setup'

export default function(createFn, setup, dismantle) {
  const db = dbSetup()
  const testPort = 30023
  const testUrl = `http://localhost:${testPort}`
  const invalidId = 'invalid-id'
  const randomId = mongoose.Types.ObjectId().toHexString()
  const updateMethods = ['PATCH', 'POST', 'PUT']

  describe('preMiddleware/Create/Read/Update/Delete - null', () => {
    let app = createFn()
    let server
    let customer

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, {
          preMiddleware: null,
          preCreate: null,
          preRead: null,
          preUpdate: null,
          preDelete: null,
          restify: app.isRestify
        })

        db.models.Customer.create({
          name: 'Bob'
        })
          .then(createdCustomer => {
            customer = createdCustomer
            server = app.listen(testPort, done)
          })
          .catch(done)
      })
    })

    afterEach(done => {
      dismantle(app, server, done)
    })

    it('POST /Customer 201', done => {
      post(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: {
            name: 'John'
          }
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 201)
          done()
        }
      )
    })

    it('GET /Customer 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          done()
        }
      )
    })

    updateMethods.forEach(method => {
      it(`${method} /Customer/:id 200`, done => {
        request(
          {
            method,
            url: `${testUrl}/api/v1/Customer/${customer._id}`,
            json: {
              name: 'Bobby'
            }
          },
          (err, res, body) => {
            ok(!err)
            equal(res.statusCode, 200)
            done()
          }
        )
      })
    })

    it('DELETE /Customer/:id 204', done => {
      del(
        {
          url: `${testUrl}/api/v1/Customer/${customer._id}`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 204)
          done()
        }
      )
    })
  })

  describe('preMiddleware', () => {
    let app = createFn()
    let server
    let customer
    let options = {
      preMiddleware: spy((req, res, next) => {
        next()
      }),
      restify: app.isRestify
    }

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, options)

        db.models.Customer.create({
          name: 'Bob'
        })
          .then(createdCustomer => {
            customer = createdCustomer
            server = app.listen(testPort, done)
          })
          .catch(done)
      })
    })

    afterEach(done => {
      options.preMiddleware.resetHistory()
      dismantle(app, server, done)
    })

    it('GET /Customer 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          _assert.calledOnce(options.preMiddleware)
          let args = options.preMiddleware.args[0]
          equal(args.length, 3)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('GET /Customer/:id 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          _assert.calledOnce(options.preMiddleware)
          let args = options.preMiddleware.args[0]
          equal(args.length, 3)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('POST /Customer 201', done => {
      post(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: {
            name: 'Pre'
          }
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 201)
          _assert.calledOnce(options.preMiddleware)
          let args = options.preMiddleware.args[0]
          equal(args.length, 3)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('POST /Customer 400 - not called (missing content type)', done => {
      post(
        {
          url: `${testUrl}/api/v1/Customer`
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 400)
          deepEqual(JSON.parse(body), {
            name: 'Error',
            message: 'missing_content_type'
          })
          _assert.notCalled(options.preMiddleware)
          done()
        }
      )
    })

    it('POST /Customer 400 - not called (invalid content type)', done => {
      post(
        {
          url: `${testUrl}/api/v1/Customer`,
          formData: {}
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 400)
          deepEqual(JSON.parse(body), {
            name: 'Error',
            message: 'invalid_content_type'
          })
          _assert.notCalled(options.preMiddleware)
          done()
        }
      )
    })

    updateMethods.forEach(method => {
      it(`${method} /Customer/:id 200`, done => {
        request(
          {
            method,
            url: `${testUrl}/api/v1/Customer/${customer._id}`,
            json: {
              name: 'Bobby'
            }
          },
          (err, res, body) => {
            ok(!err)
            equal(res.statusCode, 200)
            _assert.calledOnce(options.preMiddleware)
            let args = options.preMiddleware.args[0]
            equal(args.length, 3)
            equal(typeof args[2], 'function')
            done()
          }
        )
      })

      it(`${method} /Customer/:id 400 - not called (missing content type)`, done => {
        request(
          {
            method,
            url: `${testUrl}/api/v1/Customer/${customer._id}`
          },
          (err, res, body) => {
            ok(!err)
            equal(res.statusCode, 400)
            deepEqual(JSON.parse(body), {
              name: 'Error',
              message: 'missing_content_type'
            })
            _assert.notCalled(options.preMiddleware)
            done()
          }
        )
      })

      it(`${method} /Customer/:id 400 - not called (invalid content type)`, done => {
        request(
          {
            method,
            url: `${testUrl}/api/v1/Customer/${customer._id}`,
            formData: {}
          },
          (err, res, body) => {
            ok(!err)
            equal(res.statusCode, 400)
            deepEqual(JSON.parse(body), {
              name: 'Error',
              message: 'invalid_content_type'
            })
            _assert.notCalled(options.preMiddleware)
            done()
          }
        )
      })
    })

    it('DELETE /Customer 204', done => {
      del(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 204)
          _assert.calledOnce(options.preMiddleware)
          let args = options.preMiddleware.args[0]
          equal(args.length, 3)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('DELETE /Customer/:id 204', done => {
      del(
        {
          url: `${testUrl}/api/v1/Customer/${customer._id}`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 204)
          _assert.calledOnce(options.preMiddleware)
          let args = options.preMiddleware.args[0]
          equal(args.length, 3)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })
  })

  describe('preCreate', () => {
    let app = createFn()
    let server
    let options = {
      preCreate: spy((req, res, next) => {
        next()
      }),
      restify: app.isRestify
    }

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, options)

        server = app.listen(testPort, done)
      })
    })

    afterEach(done => {
      options.preCreate.resetHistory()
      dismantle(app, server, done)
    })

    it('POST /Customer 201', done => {
      post(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: {
            name: 'Bob'
          }
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 201)
          _assert.calledOnce(options.preCreate)
          let args = options.preCreate.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result.name, 'Bob')
          equal(args[0].erm.statusCode, 201)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })
  })

  describe('preRead', () => {
    let app = createFn()
    let server
    let customer
    let options = {
      preRead: spy((req, res, next) => {
        next()
      }),
      restify: app.isRestify
    }

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, options)

        db.models.Customer.create({
          name: 'Bob'
        })
          .then(createdCustomer => {
            customer = createdCustomer
            server = app.listen(testPort, done)
          })
          .catch(done)
      })
    })

    afterEach(done => {
      options.preRead.resetHistory()
      dismantle(app, server, done)
    })

    it('GET /Customer 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          _assert.calledOnce(options.preRead)
          let args = options.preRead.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result[0].name, 'Bob')
          equal(args[0].erm.statusCode, 200)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('GET /Customer/count 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer/count`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          _assert.calledOnce(options.preRead)
          let args = options.preRead.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result.count, 1)
          equal(args[0].erm.statusCode, 200)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('GET /Customer/:id 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer/${customer._id}`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          _assert.calledOnce(options.preRead)
          let args = options.preRead.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result.name, 'Bob')
          equal(args[0].erm.statusCode, 200)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('GET /Customer/:id/shallow 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer/${customer._id}/shallow`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          _assert.calledOnce(options.preRead)
          let args = options.preRead.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result.name, 'Bob')
          equal(args[0].erm.statusCode, 200)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })
  })

  describe('preUpdate', () => {
    let app = createFn()
    let server
    let customer
    let options = {
      preUpdate: spy((req, res, next) => {
        next()
      }),
      restify: app.isRestify
    }

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, options)

        db.models.Customer.create({
          name: 'Bob'
        })
          .then(createdCustomer => {
            customer = createdCustomer
            server = app.listen(testPort, done)
          })
          .catch(done)
      })
    })

    afterEach(done => {
      options.preUpdate.resetHistory()
      dismantle(app, server, done)
    })

    updateMethods.forEach(method => {
      it(`${method} /Customer/:id 200`, done => {
        request(
          {
            method,
            url: `${testUrl}/api/v1/Customer/${customer._id}`,
            json: {
              name: 'Bobby'
            }
          },
          (err, res, body) => {
            ok(!err)
            equal(res.statusCode, 200)
            _assert.calledOnce(options.preUpdate)
            let args = options.preUpdate.args[0]
            equal(args.length, 3)
            equal(args[0].erm.result.name, 'Bobby')
            equal(args[0].erm.statusCode, 200)
            equal(typeof args[2], 'function')
            done()
          }
        )
      })

      it(`${method} /Customer/:id 400 - not called (missing content type)`, done => {
        request(
          {
            method,
            url: `${testUrl}/api/v1/Customer/${customer._id}`
          },
          (err, res, body) => {
            ok(!err)
            equal(res.statusCode, 400)
            deepEqual(JSON.parse(body), {
              name: 'Error',
              message: 'missing_content_type'
            })
            _assert.notCalled(options.preUpdate)
            done()
          }
        )
      })

      it(`${method} /Customer/:id 400 - not called (invalid content type)`, done => {
        request(
          {
            method,
            url: `${testUrl}/api/v1/Customer/${customer._id}`,
            formData: {}
          },
          (err, res, body) => {
            ok(!err)
            equal(res.statusCode, 400)
            deepEqual(JSON.parse(body), {
              name: 'Error',
              message: 'invalid_content_type'
            })
            _assert.notCalled(options.preUpdate)
            done()
          }
        )
      })
    })
  })

  describe('preDelete', () => {
    let app = createFn()
    let server
    let customer
    let options = {
      preDelete: spy((req, res, next) => {
        next()
      }),
      restify: app.isRestify
    }

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, options)

        db.models.Customer.create({
          name: 'Bob'
        })
          .then(createdCustomer => {
            customer = createdCustomer
            server = app.listen(testPort, done)
          })
          .catch(done)
      })
    })

    afterEach(done => {
      options.preDelete.resetHistory()
      dismantle(app, server, done)
    })

    it('DELETE /Customer 204', done => {
      del(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 204)
          _assert.calledOnce(options.preDelete)
          let args = options.preDelete.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result, undefined)
          equal(args[0].erm.statusCode, 204)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('DELETE /Customer/:id 204', done => {
      del(
        {
          url: `${testUrl}/api/v1/Customer/${customer._id}`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 204)
          _assert.calledOnce(options.preDelete)
          let args = options.preDelete.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result, undefined)
          equal(args[0].erm.statusCode, 204)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })
  })

  describe('postCreate/Read/Update/Delete - null', () => {
    let app = createFn()
    let server
    let customer

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, {
          postCreate: null,
          postRead: null,
          postUpdate: null,
          postDelete: null,
          restify: app.isRestify
        })

        db.models.Customer.create({
          name: 'Bob'
        })
          .then(createdCustomer => {
            customer = createdCustomer
            server = app.listen(testPort, done)
          })
          .catch(done)
      })
    })

    afterEach(done => {
      dismantle(app, server, done)
    })

    it('POST /Customer 201', done => {
      post(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: {
            name: 'John'
          }
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 201)
          done()
        }
      )
    })

    it('GET /Customer 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          done()
        }
      )
    })

    updateMethods.forEach(method => {
      it(`${method} /Customer/:id 200`, done => {
        post(
          {
            url: `${testUrl}/api/v1/Customer/${customer._id}`,
            json: {
              name: 'Bobby'
            }
          },
          (err, res, body) => {
            ok(!err)
            equal(res.statusCode, 200)
            done()
          }
        )
      })
    })

    it('DELETE /Customer/:id 204', done => {
      del(
        {
          url: `${testUrl}/api/v1/Customer/${customer._id}`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 204)
          done()
        }
      )
    })
  })

  describe('postCreate', () => {
    let app = createFn()
    let server
    let options = {
      postCreate: spy((req, res, next) => {
        next()
      }),
      restify: app.isRestify
    }

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, options)

        server = app.listen(testPort, done)
      })
    })

    afterEach(done => {
      options.postCreate.resetHistory()
      dismantle(app, server, done)
    })

    it('POST /Customer 201', done => {
      post(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: {
            name: 'Bob'
          }
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 201)
          _assert.calledOnce(options.postCreate)
          let args = options.postCreate.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result.name, 'Bob')
          equal(args[0].erm.statusCode, 201)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('POST /Customer 400 - missing required field', done => {
      post(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: {
            comment: 'Bar'
          }
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 400)
          deepEqual(body, {
            name: 'ValidationError',
            _message: 'Customer validation failed',
            message: 'Customer validation failed: name: Path `name` is required.',
            errors: {
              name: {
                kind: 'required',
                message: 'Path `name` is required.',
                name: 'ValidatorError',
                path: 'name',
                properties: {
                  message: 'Path `name` is required.',
                  path: 'name',
                  type: 'required'
                }
              }
            }
          })
          _assert.notCalled(options.postCreate)
          done()
        }
      )
    })
  })

  describe('postRead', () => {
    let app = createFn()
    let server
    let customer
    let options = {
      postRead: spy((req, res, next) => {
        next()
      }),
      restify: app.isRestify
    }

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, options)

        db.models.Customer.create({
          name: 'Bob'
        })
          .then(createdCustomer => {
            customer = createdCustomer
            server = app.listen(testPort, done)
          })
          .catch(done)
      })
    })

    afterEach(done => {
      options.postRead.resetHistory()
      dismantle(app, server, done)
    })

    it('GET /Customer 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          _assert.calledOnce(options.postRead)
          let args = options.postRead.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result[0].name, 'Bob')
          equal(args[0].erm.statusCode, 200)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('GET /Customer/count 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer/count`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          _assert.calledOnce(options.postRead)
          let args = options.postRead.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result.count, 1)
          equal(args[0].erm.statusCode, 200)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('GET /Customer/:id 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer/${customer._id}`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          _assert.calledOnce(options.postRead)
          let args = options.postRead.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result.name, 'Bob')
          equal(args[0].erm.statusCode, 200)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('GET /Customer/:id 404', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer/${randomId}`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 404)
          _assert.notCalled(options.postRead)
          done()
        }
      )
    })

    it('GET /Customer/:id 404 - invalid id', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer/${invalidId}`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 404)
          _assert.notCalled(options.postRead)
          done()
        }
      )
    })

    it('GET /Customer/:id/shallow 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer/${customer._id}/shallow`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          _assert.calledOnce(options.postRead)
          let args = options.postRead.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result.name, 'Bob')
          equal(args[0].erm.statusCode, 200)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })
  })

  describe('postUpdate', () => {
    let app = createFn()
    let server
    let customer
    let options = {
      postUpdate: spy((req, res, next) => {
        next()
      }),
      restify: app.isRestify
    }

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, options)

        db.models.Customer.create({
          name: 'Bob'
        })
          .then(createdCustomer => {
            customer = createdCustomer
            server = app.listen(testPort, done)
          })
          .catch(done)
      })
    })

    afterEach(done => {
      options.postUpdate.resetHistory()
      dismantle(app, server, done)
    })

    updateMethods.forEach(method => {
      it(`${method} /Customer/:id 200`, done => {
        request(
          {
            method,
            url: `${testUrl}/api/v1/Customer/${customer._id}`,
            json: {
              name: 'Bobby'
            }
          },
          (err, res, body) => {
            ok(!err)
            equal(res.statusCode, 200)
            _assert.calledOnce(options.postUpdate)
            let args = options.postUpdate.args[0]
            equal(args.length, 3)
            equal(args[0].erm.result.name, 'Bobby')
            equal(args[0].erm.statusCode, 200)
            equal(typeof args[2], 'function')
            done()
          }
        )
      })

      it(`${method} /Customer/:id 404 - random id`, done => {
        request(
          {
            method,
            url: `${testUrl}/api/v1/Customer/${randomId}`,
            json: {
              name: 'Bobby'
            }
          },
          (err, res, body) => {
            ok(!err)
            equal(res.statusCode, 404)
            _assert.notCalled(options.postUpdate)
            done()
          }
        )
      })

      it(`${method} /Customer/:id 404 - invalid id`, done => {
        request(
          {
            method,
            url: `${testUrl}/api/v1/Customer/${invalidId}`,
            json: {
              name: 'Bobby'
            }
          },
          (err, res, body) => {
            ok(!err)
            equal(res.statusCode, 404)
            _assert.notCalled(options.postUpdate)
            done()
          }
        )
      })

      it(`${method} /Customer/:id 400 - not called (missing content type)`, done => {
        request(
          {
            method,
            url: `${testUrl}/api/v1/Customer/${customer._id}`
          },
          (err, res, body) => {
            ok(!err)
            equal(res.statusCode, 400)
            deepEqual(JSON.parse(body), {
              name: 'Error',
              message: 'missing_content_type'
            })
            _assert.notCalled(options.postUpdate)
            done()
          }
        )
      })

      it(`${method} /Customer/:id 400 - not called (invalid content type)`, done => {
        request(
          {
            method,
            url: `${testUrl}/api/v1/Customer/${customer._id}`,
            formData: {}
          },
          (err, res, body) => {
            ok(!err)
            equal(res.statusCode, 400)
            deepEqual(JSON.parse(body), {
              name: 'Error',
              message: 'invalid_content_type'
            })
            _assert.notCalled(options.postUpdate)
            done()
          }
        )
      })
    })
  })

  describe('postDelete', () => {
    let app = createFn()
    let server
    let customer
    let options = {
      postDelete: spy((req, res, next) => {
        next()
      }),
      restify: app.isRestify
    }

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, options)

        db.models.Customer.create({
          name: 'Bob'
        })
          .then(createdCustomer => {
            customer = createdCustomer
            server = app.listen(testPort, done)
          })
          .catch(done)
      })
    })

    afterEach(done => {
      options.postDelete.resetHistory()
      dismantle(app, server, done)
    })

    it('DELETE /Customer 204', done => {
      del(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 204)
          _assert.calledOnce(options.postDelete)
          let args = options.postDelete.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result, undefined)
          equal(args[0].erm.statusCode, 204)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('DELETE /Customer/:id 204', done => {
      del(
        {
          url: `${testUrl}/api/v1/Customer/${customer._id}`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 204)
          _assert.calledOnce(options.postDelete)
          let args = options.postDelete.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result, undefined)
          equal(args[0].erm.statusCode, 204)
          equal(typeof args[2], 'function')
          done()
        }
      )
    })

    it('DELETE /Customer/:id 404', done => {
      del(
        {
          url: `${testUrl}/api/v1/Customer/${randomId}`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 404)
          _assert.notCalled(options.postDelete)
          done()
        }
      )
    })

    it('DELETE /Customer/:id 404 - invalid id', done => {
      del(
        {
          url: `${testUrl}/api/v1/Customer/${invalidId}`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 404)
          _assert.notCalled(options.postDelete)
          done()
        }
      )
    })
  })

  describe('postCreate yields an error', () => {
    let app = createFn()
    let server
    let options = {
      postCreate: spy((req, res, next) => {
        next(new Error('Something went wrong'))
      }),
      postProcess: spy(),
      restify: app.isRestify
    }

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, options)

        server = app.listen(testPort, done)
      })
    })

    afterEach(done => {
      options.postCreate.resetHistory()
      dismantle(app, server, done)
    })

    // TODO: This test is weird
    it('POST /Customer 201', done => {
      post(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: {
            name: 'Bob'
          }
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 400)
          _assert.calledOnce(options.postCreate)
          let args = options.postCreate.args[0]
          equal(args.length, 3)
          equal(args[0].erm.result.name, 'Bob')
          equal(args[0].erm.statusCode, 400)
          equal(typeof args[2], 'function')
          _assert.notCalled(options.postProcess)
          done()
        }
      )
    })
  })

  describe('postProcess', () => {
    let app = createFn()
    let server
    let options = {
      postProcess: spy(),
      restify: app.isRestify
    }

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, options)

        server = app.listen(testPort, done)
      })
    })

    afterEach(done => {
      options.postProcess.resetHistory()
      dismantle(app, server, done)
    })

    it('GET /Customer 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          _assert.calledOnce(options.postProcess)
          let args = options.postProcess.args[0]
          equal(args.length, 2)
          deepEqual(args[0].erm.result, [])
          equal(args[0].erm.statusCode, 200)
          done()
        }
      )
    })
  })

  describe('postProcess (async outputFn)', () => {
    let app = createFn()
    let server
    let options = {
      outputFn: (req, res) => {
        if (app.isRestify) {
          res.send(200)
        } else {
          res.sendStatus(200)
        }

        return Promise.resolve()
      },
      postProcess: spy(),
      restify: app.isRestify
    }

    beforeEach(done => {
      setup(err => {
        if (err) {
          return done(err)
        }

        erm.serve(app, db.models.Customer, options)

        server = app.listen(testPort, done)
      })
    })

    afterEach(done => {
      options.postProcess.resetHistory()
      dismantle(app, server, done)
    })

    it('GET /Customer 200', done => {
      get(
        {
          url: `${testUrl}/api/v1/Customer`,
          json: true
        },
        (err, res, body) => {
          ok(!err)
          equal(res.statusCode, 200)
          _assert.calledOnce(options.postProcess)
          let args = options.postProcess.args[0]
          equal(args.length, 2)
          deepEqual(args[0].erm.result, [])
          equal(args[0].erm.statusCode, 200)
          done()
        }
      )
    })
  })
}
